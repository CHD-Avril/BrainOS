...
BrainOS 是一个面向答辩演示的 AI 企业知识库。它支持真实文档导入、千问 Embedding、Chroma 向量检索、千问/DeepSeek 问答、来源引用、用户管理和安全审计。项目提供 PC Web 与微信小程序两个客户端，两端共用同一套 Spring Boot API，同时保持单体架构易于讲解。
...
flowchart LR
    ...
    MiniProgram["uni-app + Vue 3 微信小程序"] -->|HTTPS JSON / SSE / Upload| API
    API --> MySQL[(MySQL 8.4)]
    API --> Redis[(Redis 7.4)]
    API --> Chroma[(Chroma)]
...
| 层次 | 主要技术 |
| --- | --- |
| 前端 | PC Web：Vue 3、Element Plus、ECharts；微信小程序：uni-app、Vue 3、Pinia |
...
小程序复用同一后端，但真机必须通过已在微信公众平台登记的 HTTPS 域名访问，不能请求 `localhost`：
...
在微信开发者工具中导入 `miniprogram/dist/build/mp-weixin`。AppID、真实 API 域名与证书仅放在本地/部署环境；详细配置与真机清单见 [miniprogram/README.md](miniprogram/README.md)。
...
cd miniprogram && pnpm test && pnpm typecheck && pnpm build:mp-weixin
