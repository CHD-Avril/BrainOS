...
BrainOS 是一个面向答辩演示的 AI 企业知识库。它支持真实文档导入、千问 Embedding、Chroma 向量检索、千问/DeepSeek/ChatGPT 问答、来源引用、用户管理和安全审计。项目只实现 PC Web，功能比基础示例更完整，同时保持单体架构易于讲解。
...
- 千问默认问答，DeepSeek、ChatGPT 可选，SSE 流式输出、停止/重试和文档引用。
...
| 模型 | 千问 Embedding/Chat，DeepSeek Chat，OpenAI ChatGPT |
...
- ChatGPT 无法回答：检查 `OPENAI_API_KEY`，或切回默认千问。
